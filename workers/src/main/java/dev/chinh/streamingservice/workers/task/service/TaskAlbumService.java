...
@Service
public class TaskAlbumService extends TaskMediaService {

    private final TaskVideoService taskVideoService;

    public TaskAlbumService(StringRedisTemplate redisTemplate,
                            RedissonClient redissonClient,
                            ObjectMapper objectMapper,
                            MediaMapper mediaMapper,
                            MediaMetaDataRepository mediaRepository,
                            TaskVideoService taskVideoService) {
        super(redisTemplate, redissonClient, objectMapper, mediaMapper, mediaRepository);
        this.taskVideoService = taskVideoService;
    }
    ...
    public void addCacheAlbumVideoLastAccess(long albumId, String albumVidJobId, Resolution albumRes) {
        long now = System.currentTimeMillis() + expirySeconds * 1000;
        taskVideoService.addCacheVideoLastAccess(albumVidJobId, now);
