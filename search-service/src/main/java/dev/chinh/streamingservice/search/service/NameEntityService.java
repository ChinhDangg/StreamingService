...
public class NameEntityService {
    ...
    public SearchResponse<Object> searchMatchAll(String indexName, String userId, int page, SortBy sortBy, SortOrder sortOrder) {
        ...
        try {
            return searchService.searchMatchAll(indexName, Long.parseLong(userId), page, pageSize, sortBy, sortOrder);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private <T> SearchResponse<T> searchContaining(String userId, String index, String field, String text, Class<T> clazz) throws IOException {
        return searchService.searchContaining(Long.parseLong(userId), index, field, text, clazz);
    }
