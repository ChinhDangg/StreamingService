...
//        });
    ...
    public <T> SearchResponse<T> findById(String index, long userId, long mediaId, Class<T> clazz) throws IOException {
        return client.search(s -> s
                        .index(index)
                        .query(q -> q
                                .bool(b -> b
                                        .filter(buildTermOrMatch(ContentMetaData.USER_ID, FieldValue.of(userId), true))
                                        .filter(buildTermOrMatch(ContentMetaData.ID, FieldValue.of(mediaId), true))
                                )
                        ),
                clazz
        );
