...
public interface MediaMetaDataRepository extends JpaRepository<MediaMetaData, Long> {
    ...
    @Query("""
        SELECT new dev.chinh.streamingservice.backend.modify.NameEntityDTO(a.id, a.name)
    ...
    @Query("""
        SELECT new dev.chinh.streamingservice.backend.modify.NameEntityDTO(c.id, c.name)
        FROM MediaMetaData m
        JOIN m.characters c
        WHERE m.id = :mediaId
    """)
    ...
    @Query("""
        SELECT new dev.chinh.streamingservice.backend.modify.NameEntityDTO(u.id, u.name)
        FROM MediaMetaData m
        JOIN m.universes u
        WHERE m.id = :mediaId
    """)
    ...
    @Query("""
        SELECT new dev.chinh.streamingservice.backend.modify.NameEntityDTO(t.id, t.name)
        FROM MediaMetaData m
        JOIN m.tags t
        WHERE m.id = :mediaId
    """)
