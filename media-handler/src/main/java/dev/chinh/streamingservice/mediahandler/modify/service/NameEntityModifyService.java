...
import dev.chinh.streamingservice.mediahandler.modify.dto.NameAndThumbnailPostRequest;
...
public class NameEntityModifyService {
    ...
    protected <T extends MediaNameEntity> void updateNameEntity(String userId, long id, String name,
        ...
        eventPublisher.publishEvent(new MediaHandlerEventProducer.EventWrapper(
                EventTopics.MEDIA_SEARCH_TOPIC,
                userId,
                new MediaUpdateEvent.NameEntityUpdated(userId, mediaNameEntityConstant, id, name, null, null)
        ));
    }
        ...
        try {
            ...
            if (nameChanged || thumbnailChanged) {
                ...
                eventPublisher.publishEvent(new MediaHandlerEventProducer.EventWrapper(
                        topic,
                        userId,
                        new MediaUpdateEvent.NameEntityUpdated(userId, mediaNameEntityConstant, id, nameChanged ? newName : null, oldThumbnailPath, newThumbnailPath)
                ));
                if (thumbnailChanged)
                    eventPublisher.publishEvent(new MediaHandlerEventProducer.EventWrapper(
